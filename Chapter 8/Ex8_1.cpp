// Modify the calculator program to make the input stream an explicit parameter. Also give the
// Token_stream constructor an istream& parameter.
// Removed Symbol_table st from global variable.
...
/*
          ...
          Declaration:
                    'let' Name '=' Expression
                    'const' non_Existing_variable '-' Expression
...
public:
    ...
    Token_stream(istream& is) :ist{is} {}       // initialize Token_stream with istream
private:
...
double expression(Token_stream&, Symbol_table& st);  // read and evaluate a Expression

double term(Token_stream&, Symbol_table& st);        // read and evaluate a Term

//------------------------------------------------------------------------------

double primary(Token_stream& ts, Symbol_table& st)     // read and evaluate a Primary
{
    Token t = ts.get();
...
double declaration(Token_stream& ts, Symbol_table& st, bool b)
...
double statement(Token_stream& ts, Symbol_table& st)
{
    ...
    switch (t.kind) {
    case chConst:
        return declaration(ts, st, true);       // const declaration
    case let:
        return declaration(ts, st, false);      // var declaration
    default:
        ts.putback(t);
...
void calculate(Symbol_table& st)        // expression evaluation loop
{
...
{
    double left = term(ts, st);      // read and evaluate a Term
    Token t = ts.get();     // get the next token

    while (true) {
        switch (t.kind) {
        case '+':
            left += term(ts, st);    // evaluate Term and add
            t = ts.get();
            break;
        case '-':
            left -= term(ts, st);    // evaluate Term and subtract
            t = ts.get();
            break;
        default:
