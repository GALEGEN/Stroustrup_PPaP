// Программа, ноходящая все простые числа между 1 и max через классический метод "Решето Эратосфена".
...
std::vector<bool> primeSearch(const int& max_limit)
// реализация алгоритма Решета Эратосфена по псевдокоду
// например проверяется промежуток чисел от 0 до 100 => numbers.size() == 101
{
    int search_limit{ max_limit + 1 };
...
int main()
{
    ...
    std::vector<bool> answer = primeSearch(max);

    std::cout << "Простые числа от 1 до " << max << ": \n";
    primePrint(answer);
}
